...
const VideoCard: React.FC<VideoCardProps> = memo(
  ({
    ...
    const progressBarBottom =
      visualTabIconsHeight + insets.bottom + spacingAboveTabBar;
    ...
    const sendWatchInteraction = useCallback(() => {
      const resolvedDuration = getResolvedDuration();
      if (!userDetailIdRef.current || !resolvedDuration) {
        return;
      }
      ...
      UserInteractionService.saveInteraction({
        ...
        watchDuration: resolvedDuration,
      })
        .then(() => {
    ...
    }, [isActive, player, sendWatchInteraction]);
    ...
    const videoContainerHeight = isAnyModalOpen
                    ...
                    style={[
                      styles.progressThumb,
                      {
                        left: Math.max(
                          ...
                          Math.min(
                            progressBarWidth -
                              theme.componentSizes.progressBar.thumbSize / 2,
...
const styles = StyleSheet.create({
  ...
  progressThumb: {
    ...
    top:
      -(
        theme.componentSizes.progressBar.thumbSize -
        theme.componentSizes.progressBar.height
      ) / 2,
