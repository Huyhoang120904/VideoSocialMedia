...
export const fetchFeedItems = async (
  ...
  try {
    const { data } =
      await api.get<ApiResponse<PaginatedResponse<FeedItemResponse>>>(
        "/feed/personal"
      );
    ...
    console.log(
      "Full response:",
      JSON.stringify(data.result?.content, null, 2)
    );
      ...
      data.result?.content
        ?.map((feedItem: FeedItemResponse) => {
          ...
          if (
            ...
            console.log(
              "ImageSlide raw data:",
              JSON.stringify(feedItem.imageSlide, null, 2)
            );
            ...
            const item: FeedItem = {
              ...
              outstanding: 0,
              title:
                feedItem.title || imageSlide.images[0]?.title || "Image Slide",
              description:
                feedItem.description ||
                imageSlide.images[0]?.description ||
                "Image Collection",
              hashtags: feedItem.hashTags || [],
              loved: feedItem.loved || false, // Thêm trạng thái loved
              uploader: feedItem.uploader, // Thông tin người upload
    ...
    if (
      ...
      console.log(`Retrying fetchFeedItems in ${(retryCount + 1) * 1000}ms...`);
